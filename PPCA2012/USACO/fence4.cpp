...
int n,rx,ry,r[203],belong[40001];
pair<int,int> p[203];
...
bool cmp(int x,int y){/*
	...
	else return max(dis(make_pair(rx,ry),p[x]),dis(make_pair(rx,ry),p[x+1]))
		<max(dis(make_pair(rx,ry),p[y]),dis(make_pair(rx,ry),p[y+1]));
	return calc(make_pair(rx,ry),p[x],p[x+1])<calc(make_pair(rx,ry),p[y],p[y+1]);
}
